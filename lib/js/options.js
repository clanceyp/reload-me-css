...
(function($, chrome, document, window){
    ...
    options = {
        clearLocalStore:function(){
            document.clearLocalStore();
        },
        setLocalStore: function(key, value){
            document.setLocalStore(key, value);
        },
        getLocalStore: function(key, fallback){
            var value = documen.getLocalStore( key );
         ...
         load: function(){
            function saveFunction(e){
                ...
                if (self.type === "checkbox"){
                    options.setLocalStore(self.key,  self.checked);
                }else{
                    options.setLocalStore(self.key,  self.value);
            ...
            for (var item in editableOptions){
                if (editableOptions.hasOwnProperty(item)){
                    var option = editableOptions[item],
                        value = options.getLocalStore( option.key ),
            ...
            for (var item in editableOptions){
                if (editableOptions.hasOwnProperty(item)){
                    ...
                    if (option.type === "text" || option.type.match("select")) {
                        ...
                        options.setLocalStore( option.key ,  value);
                    } else if (option.type === "checkbox"){
                        options.setLocalStore( option.key ,  document.getElementById( option.id ).checked);
                    }else {
                        console.log("Not implemented option type: " + option.type );
                    }
                }
